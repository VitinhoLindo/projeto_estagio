...
export default {
  ...
  async mounted() {
     ...
     *   <summary>
     *      call to server in route ['/collaborators']
    ...
    this.$app.on('resize', this.handlePage, (err, pid) => {
      ...
      this.collaboratorsPid = pid;
    });
    await this.getData();
  },
  data() {
    return {
      collaboratorsPid: null,
  ...
  methods: {
    ...
    async getData(count = 0, error = '') {
      if (count == 5) {
        ...
        return this.$app.emit('error', { on: error, show: true });
      }
     ...
     * </summary>
    */
    this.$app.removeListiner('resize', this.collaboratorsPid);
  }
}
</script>
